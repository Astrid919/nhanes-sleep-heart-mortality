...
categorical <- c(
  ...
  "AlcoholUse", "Hypertension", "Diabetes", "Stroke", "CVD"
)
level_labels <- list(
  ...
  CVD = c("0" = "Baseline coronary heart disease: no", "1" = "Baseline coronary heart disease: yes")
)

rows <- list()
rows[[length(rows) + 1]] <- data.frame(
  Characteristic = "Unweighted n", Level = "",
  t(as.matrix(table(primary$SleepClinical5)[group_levels])),
  check.names = FALSE
)
names(rows[[1]]) <- c("Characteristic", "Level", group_levels)
rows[[length(rows) + 1]] <- data.frame(
  Characteristic = "Heart disease deaths, n", Level = "",
  t(vapply(group_levels, function(g) sum(primary$event[primary$SleepClinical5 == g] == 1), integer(1))),
  check.names = FALSE
)
names(rows[[2]]) <- c("Characteristic", "Level", group_levels)
