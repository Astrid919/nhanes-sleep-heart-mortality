...
maxit <- 10
reuse_existing_mice <- tolower(Sys.getenv("REUSE_MICE", "false")) %in% c("true", "1", "yes")

mi_vars <- unique(c(
  "Sleeptime", "Time", "event", "WT_14YR", "SDMVPSU", "SDMVSTRA",
...
no_impute <- c("Sleeptime", "Time", "logTime", "event", "WT_14YR", "SDMVPSU", "SDMVSTRA")
...
mice_path <- file.path("outputs", "tables", paste0("mice_primary_", m, "_imputations.rds"))
if (reuse_existing_mice && file.exists(mice_path)) {
  message("Reusing existing MICE object: ", mice_path)
...
fit_mi <- function(exposure, covars, model_name) {
  ...
  for (i in seq_len(imp$m)) {
    ...
    if (!"BaselineCHD" %in% names(d) && "CVD" %in% names(d)) {
      d$BaselineCHD <- d$CVD
    }
